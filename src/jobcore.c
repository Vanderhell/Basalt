...
jobdb_result_t jobcore_worker_id(jobcore_t *c,uint32_t index,uint8_t out[16]) { jobdb_worker_id_t worker; if(!c||!out||index==0||index>c->worker_count)return JOBDB_ERR_INVALID_ARGUMENT; make_worker(c,&worker,index); memcpy(out,worker.bytes,16); return JOBDB_OK; }
...
jobdb_result_t jobcore_schedule_create_ex(jobcore_t *c, const jobcore_schedule_spec_t *spec, int64_t end_at) {
...
jobdb_result_t jobcore_schedule_create(jobcore_t *c,const jobcore_schedule_spec_t *spec){return jobcore_schedule_create_ex(c,spec,0);}
...
jobdb_result_t jobcore_enqueue_idempotent(jobcore_t*c,const char*key,uint64_t type,const void*payload,uint32_t size,uint32_t version,int64_t now,uint32_t max_attempts,uint64_t*out)
{
    enum { RECEIPT_HEADER_SIZE=48, RECEIPT_LEGACY_SIZE=40, RECEIPT_KEY_LIMIT=4096 };
    const uint32_t receipt_magic=UINT32_C(0x324D4449), receipt_version=2;
    uint64_t key_hash,fingerprint;size_t key_size,total_size;uint8_t*receipt=NULL,*buffer=NULL;uint32_t stored_size=0;jobdb_execution_t e;jobdb_result_t r;
    if(!c||!key||!*key||!type||(size&&!payload)||!out)return JOBDB_ERR_INVALID_ARGUMENT;
    key_size=strlen(key);
    if(key_size>RECEIPT_KEY_LIMIT||size>UINT32_MAX-PAYLOAD_HEADER_SIZE||key_size>UINT32_MAX-RECEIPT_HEADER_SIZE-size)return JOBDB_ERR_LIMIT;
    total_size=RECEIPT_HEADER_SIZE+key_size+(size_t)size;
    receipt=(uint8_t*)malloc(total_size);
    if(!receipt)return JOBDB_ERR_INTERNAL;
    key_hash=hash_name(key);fingerprint=core_fingerprint(type,version,payload,size);
    r=basalt_storage_idempotency_get(c->storage,key_hash,receipt,(uint32_t)total_size,&stored_size);
    if(r==JOBDB_OK){
        if(stored_size==RECEIPT_LEGACY_SIZE){
            if(core_get_u64le(receipt+8)!=key_hash||core_get_u64le(receipt+16)!=type||core_get_u64le(receipt+24)!=fingerprint)r=JOBDB_ERR_CONFLICT;
            else{*out=core_get_u64le(receipt);r=JOBDB_OK;}
        }else if(stored_size!=total_size||workflow_get_u32le(receipt)!=receipt_magic||workflow_get_u32le(receipt+4)!=receipt_version||core_get_u64le(receipt+16)!=key_hash||core_get_u64le(receipt+24)!=type||workflow_get_u32le(receipt+32)!=version||workflow_get_u32le(receipt+36)!=(uint32_t)key_size||workflow_get_u32le(receipt+40)!=size||memcmp(receipt+RECEIPT_HEADER_SIZE,key,key_size)||(size&&memcmp(receipt+RECEIPT_HEADER_SIZE+key_size,payload,size)))r=JOBDB_ERR_CONFLICT;
        else{*out=core_get_u64le(receipt+8);r=JOBDB_OK;}
        free(receipt);return r;
    }
    if(r!=JOBDB_ERR_NOT_FOUND){free(receipt);return r;}
    buffer=(uint8_t*)malloc(PAYLOAD_HEADER_SIZE+(size_t)size);
    if(!buffer){free(receipt);return JOBDB_ERR_INTERNAL;}
    memcpy(buffer,&type,8);memcpy(buffer+8,&version,4);memcpy(buffer+12,&size,4);if(size)memcpy(buffer+PAYLOAD_HEADER_SIZE,payload,size);
    memset(&e,0,sizeof e);e.execution_id=allocate_execution_id(c);
    if(!e.execution_id){free(buffer);free(receipt);return JOBDB_ERR_LIMIT;}
    e.job_definition_id=type;e.state=JOBDB_EXEC_READY;e.created_at=now;e.eligible_at=now;e.max_attempts=max_attempts?max_attempts:1;
    memset(receipt,0,total_size);workflow_put_u32le(receipt,receipt_magic);workflow_put_u32le(receipt+4,receipt_version);core_put_u64le(receipt+8,e.execution_id);core_put_u64le(receipt+16,key_hash);core_put_u64le(receipt+24,type);workflow_put_u32le(receipt+32,version);workflow_put_u32le(receipt+36,(uint32_t)key_size);workflow_put_u32le(receipt+40,size);memcpy(receipt+RECEIPT_HEADER_SIZE,key,key_size);if(size)memcpy(receipt+RECEIPT_HEADER_SIZE+key_size,payload,size);
    r=basalt_storage_execution_enqueue_receipt(c->storage,&e,PAYLOAD_RECORD_TYPE,buffer,PAYLOAD_HEADER_SIZE+size,key_hash,receipt,(uint32_t)total_size);
    free(buffer);
    if(r==JOBDB_ERR_ALREADY_EXISTS){
        stored_size=0;r=basalt_storage_idempotency_get(c->storage,key_hash,receipt,(uint32_t)total_size,&stored_size);
        if(r==JOBDB_OK&&stored_size==total_size&&workflow_get_u32le(receipt)==receipt_magic&&core_get_u64le(receipt+16)==key_hash&&core_get_u64le(receipt+24)==type&&workflow_get_u32le(receipt+32)==version&&workflow_get_u32le(receipt+36)==key_size&&workflow_get_u32le(receipt+40)==size&&!memcmp(receipt+RECEIPT_HEADER_SIZE,key,key_size)&&(!size||!memcmp(receipt+RECEIPT_HEADER_SIZE+key_size,payload,size))){*out=core_get_u64le(receipt+8);r=JOBDB_OK;}
        else if(r==JOBDB_OK)r=JOBDB_ERR_CONFLICT;
    }
    free(receipt);*out=e.execution_id;return r;
}
jobdb_result_t jobcore_enqueue_idempotent_with_retry(jobcore_t*c,const char*key,uint64_t type,const void*payload,uint32_t size,uint32_t version,int64_t now,const jobcore_retry_spec_t*spec,uint64_t*out)
{
    enum { RH=48, KEY_LIMIT=4096 };const uint32_t magic=UINT32_C(0x324D4449);size_t kn,total;uint64_t kh;uint8_t *receipt=NULL,*body=NULL,retry_bytes[RETRY_RECORD_SIZE];uint32_t observed=0;jobdb_result_t r;jobdb_execution_t e={0};retry_record_t rr={0};jobdb_stats_t stats;jobdb_record_t sr={0};uint64_t stats_revision=0;int has_stats=0;basalt_storage_tx_t*tx=NULL;
    if(!c||!key||!*key||!type||(size&&!payload)||!spec||!out||spec->policy<JOBCORE_RETRY_FIXED||spec->policy>JOBCORE_RETRY_EXPONENTIAL_WITH_JITTER||!spec->max_attempts||spec->initial_delay<0||spec->max_delay<0||spec->jitter>86400u)return JOBDB_ERR_INVALID_ARGUMENT;
    kn=strlen(key);if(kn>KEY_LIMIT||size>UINT32_MAX-PAYLOAD_HEADER_SIZE||kn>UINT32_MAX-RH-size)return JOBDB_ERR_LIMIT;total=RH+kn+(size_t)size;kh=hash_name(key);receipt=(uint8_t*)calloc(1,total);if(!receipt)return JOBDB_ERR_INTERNAL;
    r=basalt_storage_idempotency_get(c->storage,kh,receipt,(uint32_t)total,&observed);
    if(r==JOBDB_OK){if(observed==total&&workflow_get_u32le(receipt)==magic&&workflow_get_u32le(receipt+4)==2&&core_get_u64le(receipt+16)==kh&&core_get_u64le(receipt+24)==type&&workflow_get_u32le(receipt+32)==version&&workflow_get_u32le(receipt+36)==kn&&workflow_get_u32le(receipt+40)==size&&!memcmp(receipt+RH,key,kn)&&(!size||!memcmp(receipt+RH+kn,payload,size))){*out=core_get_u64le(receipt+8);free(receipt);return JOBDB_OK;}free(receipt);return JOBDB_ERR_CONFLICT;}if(r!=JOBDB_ERR_NOT_FOUND){free(receipt);return r;}
    body=(uint8_t*)malloc(PAYLOAD_HEADER_SIZE+(size_t)size);if(!body){free(receipt);return JOBDB_ERR_INTERNAL;}memcpy(body,&type,8);memcpy(body+8,&version,4);memcpy(body+12,&size,4);if(size)memcpy(body+PAYLOAD_HEADER_SIZE,payload,size);
    e.execution_id=allocate_execution_id(c);if(!e.execution_id){free(body);free(receipt);return JOBDB_ERR_LIMIT;}e.job_definition_id=type;e.state=JOBDB_EXEC_READY;e.created_at=now;e.eligible_at=now;e.max_attempts=spec->max_attempts;
    workflow_put_u32le(receipt,magic);workflow_put_u32le(receipt+4,2);core_put_u64le(receipt+8,e.execution_id);core_put_u64le(receipt+16,kh);core_put_u64le(receipt+24,type);workflow_put_u32le(receipt+32,version);workflow_put_u32le(receipt+36,(uint32_t)kn);workflow_put_u32le(receipt+40,size);memcpy(receipt+RH,key,kn);if(size)memcpy(receipt+RH+kn,payload,size);
    rr.policy=(uint32_t)spec->policy;rr.max_attempts=spec->max_attempts;rr.initial_delay=spec->initial_delay;rr.max_delay=spec->max_delay;rr.backoff_factor=spec->backoff_factor;rr.jitter=spec->jitter;retry_encode(&rr,retry_bytes);
    r=basalt_storage_get_stats(c->storage,&stats);if(r==JOBDB_OK){stats.submitted_total++;r=basalt_storage_record_get(c->storage,5,1,&sr);if(r==JOBDB_OK){has_stats=1;stats_revision=sr.revision;basalt_storage_record_free(c->storage,&sr);}else if(r==JOBDB_ERR_NOT_FOUND)r=JOBDB_OK;}
    if(r==JOBDB_OK)r=basalt_storage_tx_begin(c->storage,&tx);if(r==JOBDB_OK)r=basalt_storage_tx_put_execution_create(tx,&e);if(r==JOBDB_OK)r=basalt_storage_tx_put_create(tx,PAYLOAD_RECORD_TYPE,e.execution_id,body,PAYLOAD_HEADER_SIZE+size);if(r==JOBDB_OK)r=basalt_storage_tx_put_create(tx,RETRY_RECORD_TYPE,e.execution_id,retry_bytes,RETRY_RECORD_SIZE);if(r==JOBDB_OK)r=basalt_storage_tx_put_create(tx,6,kh,receipt,(uint32_t)total);if(r==JOBDB_OK)r=basalt_storage_tx_put_stats(tx,&stats,stats_revision,has_stats);if(r==JOBDB_OK)r=basalt_storage_tx_commit(tx);basalt_storage_tx_rollback(tx);
    if(r==JOBDB_ERR_ALREADY_EXISTS||r==JOBDB_ERR_CONFLICT){observed=0;jobdb_result_t q=basalt_storage_idempotency_get(c->storage,kh,receipt,(uint32_t)total,&observed);if(q==JOBDB_OK&&observed==total&&workflow_get_u32le(receipt)==magic&&core_get_u64le(receipt+16)==kh&&core_get_u64le(receipt+24)==type&&workflow_get_u32le(receipt+32)==version&&workflow_get_u32le(receipt+36)==kn&&workflow_get_u32le(receipt+40)==size&&!memcmp(receipt+RH,key,kn)&&(!size||!memcmp(receipt+RH+kn,payload,size))){*out=core_get_u64le(receipt+8);r=JOBDB_OK;}else if(q==JOBDB_OK)r=JOBDB_ERR_CONFLICT;}
    free(body);free(receipt);*out=e.execution_id;return r;
}
...
jobdb_result_t jobcore_workflow_node_list(jobcore_t*c,uint64_t workflow_id,jobcore_workflow_node_view_t*out,size_t capacity,size_t*out_count){uint64_t*ids=NULL;size_t count=0,i,used=0;int found=0;if(!c||!workflow_id||!out_count||(capacity&&!out))return JOBDB_ERR_INVALID_ARGUMENT;if(!load_all_record_ids(c,WORKFLOW_NODE_RECORD_TYPE,&ids,&count))return JOBDB_ERR_IO;for(i=0;i<count;i++){jobdb_record_t r={0};workflow_record_t n;if(basalt_storage_record_get(c->storage,WORKFLOW_NODE_RECORD_TYPE,ids[i],&r)==JOBDB_OK&&workflow_decode(r.payload,r.payload_size,&n)==JOBDB_OK&&n.workflow_id==workflow_id){found=1;if(used<capacity){out[used].node_id=n.node_id;out[used].execution_id=n.execution_id;out[used].dependency_count=n.dependency_count;out[used].policy=n.policy;memcpy(out[used].dependencies,n.dependencies,sizeof n.dependencies);}used++;}if(r.payload)basalt_storage_record_free(c->storage,&r);}free(ids);*out_count=used;return found?JOBDB_OK:JOBDB_ERR_NOT_FOUND;}
