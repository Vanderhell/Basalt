...
static int workflow_cancel_requested(jobcore_t *c, uint64_t workflow_id) { jobdb_record_t r={0}; int yes=jobdb_record_get(c->db,WORKFLOW_CANCEL_RECORD_TYPE,workflow_id,&r)==JOBDB_OK; if(r.payload)jobdb_record_free(&r); return yes; }
...
jobdb_result_t jobcore_enqueue_idempotent(jobcore_t*c,const char*key,uint64_t type,const void*payload,uint32_t size,uint32_t version,int64_t now,uint32_t max_attempts,uint64_t*out){uint64_t key_hash,fingerprint;uint8_t receipt[40];uint8_t*buffer;uint32_t receipt_size=0;jobdb_execution_t e;jobdb_result_t r;if(!c||!key||!*key||!type||(size&&!payload)||!out)return JOBDB_ERR_INVALID_ARGUMENT;if(size>UINT32_MAX-PAYLOAD_HEADER_SIZE)return JOBDB_ERR_LIMIT;key_hash=hash_name(key);fingerprint=core_fingerprint(type,version,payload,size);r=jobdb_idempotency_get(c->db,key_hash,receipt,sizeof receipt,&receipt_size);if(r==JOBDB_OK){if(receipt_size!=40||core_get_u64le(receipt+8)!=key_hash||core_get_u64le(receipt+16)!=type||core_get_u64le(receipt+24)!=fingerprint)return JOBDB_ERR_CONFLICT;*out=core_get_u64le(receipt);return JOBDB_OK;}if(r!=JOBDB_ERR_NOT_FOUND)return r;buffer=(uint8_t*)malloc(PAYLOAD_HEADER_SIZE+(size_t)size);if(!buffer)return JOBDB_ERR_INTERNAL;memcpy(buffer,&type,8);memcpy(buffer+8,&version,4);memcpy(buffer+12,&size,4);if(size)memcpy(buffer+PAYLOAD_HEADER_SIZE,payload,size);memset(&e,0,sizeof e);e.execution_id=allocate_execution_id(c);if(!e.execution_id){free(buffer);return JOBDB_ERR_LIMIT;}e.job_definition_id=type;e.state=JOBDB_EXEC_READY;e.created_at=now;e.eligible_at=now;e.max_attempts=max_attempts?max_attempts:1;memset(receipt,0,sizeof receipt);core_put_u64le(receipt,e.execution_id);core_put_u64le(receipt+8,key_hash);core_put_u64le(receipt+16,type);core_put_u64le(receipt+24,fingerprint);core_put_u64le(receipt+32,version);r=jobdb_execution_enqueue_receipt(c->db,&e,PAYLOAD_RECORD_TYPE,buffer,PAYLOAD_HEADER_SIZE+size,key_hash,receipt,sizeof receipt);free(buffer);if(r==JOBDB_ERR_ALREADY_EXISTS){r=jobdb_idempotency_get(c->db,key_hash,receipt,sizeof receipt,&receipt_size);if(r==JOBDB_OK&&receipt_size==40&&core_get_u64le(receipt+16)==type&&core_get_u64le(receipt+24)==fingerprint){*out=core_get_u64le(receipt);return JOBDB_OK;}if(r==JOBDB_OK)return JOBDB_ERR_CONFLICT;}*out=e.execution_id;return r;}
jobdb_result_t jobcore_workflow_get(jobcore_t*c,uint64_t workflow_id,jobcore_workflow_status_t*out){uint64_t*ids=NULL;size_t count=0,i;int found=0;if(!c||!workflow_id||!out)return JOBDB_ERR_INVALID_ARGUMENT;memset(out,0,sizeof*out);out->workflow_id=workflow_id;if(!load_all_record_ids(c,WORKFLOW_NODE_RECORD_TYPE,&ids,&count))return JOBDB_ERR_IO;for(i=0;i<count;i++){jobdb_record_t r={0};workflow_record_t n;jobdb_execution_t e;if(jobdb_record_get(c->db,WORKFLOW_NODE_RECORD_TYPE,ids[i],&r)==JOBDB_OK&&workflow_decode(r.payload,r.payload_size,&n)==JOBDB_OK){if(n.workflow_id==workflow_id&&jobdb_execution_get(c->db,n.execution_id,&e)==JOBDB_OK){found=1;out->node_count++;if(e.state==JOBDB_EXEC_READY)out->ready_count++;else if(e.state==JOBDB_EXEC_BLOCKED)out->blocked_count++;else if(e.state==JOBDB_EXEC_LEASED||e.state==JOBDB_EXEC_RUNNING)out->running_count++;else if(e.state==JOBDB_EXEC_DONE||e.state==JOBDB_EXEC_FAILED||e.state==JOBDB_EXEC_DEAD||e.state==JOBDB_EXEC_CANCELLED){out->terminal_count++;if(e.state==JOBDB_EXEC_FAILED||e.state==JOBDB_EXEC_DEAD)out->failed_count++;if(e.state==JOBDB_EXEC_CANCELLED)out->cancelled_count++;}}}if(r.payload)jobdb_record_free(&r);}free(ids);out->cancel_requested=workflow_cancel_requested(c,workflow_id)?1u:0u;return found?JOBDB_OK:JOBDB_ERR_NOT_FOUND;}
 jobdb_result_t jobcore_workflow_cancel(jobcore_t*c,uint64_t workflow_id){uint8_t p[16]={0};jobdb_result_t r;if(!c||!workflow_id)return JOBDB_ERR_INVALID_ARGUMENT;core_put_u64le(p,1);core_put_u64le(p+8,workflow_id);r=jobdb_record_create(c->db,WORKFLOW_CANCEL_RECORD_TYPE,workflow_id,p,sizeof p);return r==JOBDB_ERR_ALREADY_EXISTS?JOBDB_OK:r;}
