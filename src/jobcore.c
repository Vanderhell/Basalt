...
jobdb_result_t jobcore_workflow_get(jobcore_t*c,uint64_t workflow_id,jobcore_workflow_status_t*out){uint64_t*ids=NULL;size_t count=0,i;int found=0;if(!c||!workflow_id||!out)return JOBDB_ERR_INVALID_ARGUMENT;memset(out,0,sizeof*out);out->workflow_id=workflow_id;if(!load_all_record_ids(c,WORKFLOW_NODE_RECORD_TYPE,&ids,&count))return JOBDB_ERR_IO;for(i=0;i<count;i++){jobdb_record_t r={0};workflow_record_t n;jobdb_execution_t e;if(jobdb_record_get(c->db,WORKFLOW_NODE_RECORD_TYPE,ids[i],&r)==JOBDB_OK&&r.payload_size==sizeof n){memcpy(&n,r.payload,sizeof n);if(n.workflow_id==workflow_id&&jobdb_execution_get(c->db,n.execution_id,&e)==JOBDB_OK){found=1;out->node_count++;if(e.state==JOBDB_EXEC_READY)out->ready_count++;else if(e.state==JOBDB_EXEC_BLOCKED)out->blocked_count++;else if(e.state==JOBDB_EXEC_LEASED||e.state==JOBDB_EXEC_RUNNING)out->running_count++;else if(e.state==JOBDB_EXEC_DONE||e.state==JOBDB_EXEC_FAILED||e.state==JOBDB_EXEC_DEAD||e.state==JOBDB_EXEC_CANCELLED){out->terminal_count++;if(e.state==JOBDB_EXEC_FAILED||e.state==JOBDB_EXEC_DEAD)out->failed_count++;if(e.state==JOBDB_EXEC_CANCELLED)out->cancelled_count++;}}}if(r.payload)jobdb_record_free(&r);}free(ids);out->cancel_requested=workflow_cancel_requested(c,workflow_id)?1u:0u;return found?JOBDB_OK:JOBDB_ERR_NOT_FOUND;}
