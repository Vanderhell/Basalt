...
{
    private const uint JobKeyRecordType = 1000, ScheduleKeyRecordType = 1001, WorkflowKeyRecordType = 1002;
    ...
    internal BasaltApplication(BasaltEngine engine,IDisposable ownedStorage){_engine=engine;_ownedStorage=ownedStorage;LoadKeys(JobKeyRecordType,_jobKeys);LoadKeys(ScheduleKeyRecordType,_scheduleKeys);LoadKeys(WorkflowKeyRecordType,_workflowKeys);}
    ...
    public BasaltStats GetStats()=>_engine.GetStats();public BasaltLedgerEntry GetLedger(ulong id)=>_engine.GetLedger(id);public void Cancel(ulong id)=>_engine.Cancel(id);public void Requeue(ulong id)=>_engine.Requeue(id);
    ...
    public BasaltWorkflowStatus GetWorkflow(ulong id)=>Decorate(_engine.GetWorkflow(id));public BasaltWorkflowStatus GetWorkflow(string key)=>GetWorkflow(JobKey.Hash("workflow:"+key));public IReadOnlyList<BasaltWorkflowStatus> ListWorkflows(int take=100,ulong afterWorkflowId=0)=>_engine.ListWorkflows(take,afterWorkflowId).Select(Decorate).ToArray();public void VerifyHealth()=>_engine.VerifyHealth();
    private void LoadKeys(uint recordType,Dictionary<ulong,string> target){foreach(var value in _engine.ListManagementText(recordType))target[value.Key]=value.Value;}
    private BasaltExecutionInfo Decorate(BasaltExecutionInfo value){if(_jobKeys.TryGetValue(value.JobDefinitionId,out var key))value.JobKey=key;return value;} private BasaltScheduleInfo Decorate(BasaltScheduleInfo value){if(_scheduleKeys.TryGetValue(value.ScheduleId,out var schedule))value.ScheduleKey=schedule;if(_jobKeys.TryGetValue(value.JobDefinitionId,out var job))value.JobKey=job;return value;}private BasaltWorkflowStatus Decorate(BasaltWorkflowStatus value){if(_workflowKeys.TryGetValue(value.WorkflowId,out var key))value.WorkflowKey=key;return value;}
