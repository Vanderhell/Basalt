...
#ifdef _WIN32
static int child_tx(const char *path, unsigned base) { jobdb_t *db=NULL; unsigned char x; assert(jobdb_open(path,&db)==JOBDB_OK); for(unsigned i=0;i<100;i++){jobdb_tx_t *tx=NULL;uint64_t id=(uint64_t)base*100+i+1;x=(unsigned char)i;assert(jobdb_tx_begin(db,&tx)==JOBDB_OK);assert(jobdb_tx_put(tx,8,id,&x,1)==JOBDB_OK);assert(jobdb_tx_commit(tx)==JOBDB_OK);jobdb_tx_rollback(tx);}jobdb_close(db);return 0; }
static int child_conflict(const char *path, unsigned v) { jobdb_t *db=NULL;jobdb_record_t r;unsigned char x=(unsigned char)v;assert(jobdb_open(path,&db)==JOBDB_OK);assert(jobdb_record_get(db,8,1,&r)==JOBDB_OK);uint64_t rev=r.revision;jobdb_record_free(&r);Sleep(100);jobdb_result_t z=jobdb_record_update(db,8,1,rev,&x,1,NULL);jobdb_close(db);return z==JOBDB_OK?0:(z==JOBDB_ERR_CONFLICT?2:3); }
...
static int spawn_wait(const char *exe, const char *a, const char *b) { const char *args[4]={exe,a,b,NULL}; intptr_t h=_spawnv(_P_NOWAIT,exe,args); assert(h!=-1); int s=0; assert(_cwait(&s,h,0)!=-1); return s; }
#endif
...
int main(int argc, char **argv) {
    ...
    if(strcmp(argv[1],"corruption")==0)return wal_test();
...
#ifdef _WIN32
    if(strcmp(argv[1],"multiprocess")==0){char path[256],num[32];jobdb_t*db=NULL;size_t n=0;uint64_t*ids;unsigned char x=1;path_for(path,sizeof path,"multiprocess");make_db(path,&db);for(unsigned p=0;p<10;p++){sprintf(num,"%u",p);const char*args[]={argv[0],"tx-child",path,num,NULL};intptr_t h=_spawnv(_P_NOWAIT,argv[0],args);assert(h!=-1);int s=0;assert(_cwait(&s,h,0)!=-1&&s==0);}assert(jobdb_list_record_ids(db,8,NULL,0,&n)==JOBDB_OK&&n==1000);ids=(uint64_t*)malloc(n*sizeof*ids);assert(ids);assert(jobdb_list_record_ids(db,8,ids,n,&n)==JOBDB_OK);free(ids);
        assert(jobdb_record_create(db,8,9001,&x,1)==JOBDB_OK);jobdb_close(db);db=NULL;{const char*args1[]={argv[0],"conflict-child",path,"1",NULL};const char*args2[]={argv[0],"conflict-child",path,"2",NULL};intptr_t h1=_spawnv(_P_NOWAIT,argv[0],args1),h2=_spawnv(_P_NOWAIT,argv[0],args2);int s1=0,s2=0;assert(h1!=-1&&h2!=-1);assert(_cwait(&s1,h1,0)!=-1&&_cwait(&s2,h2,0)!=-1);assert((s1==0&&s2==2)||(s1==2&&s2==0));}
        assert(jobdb_open(path,&db)==JOBDB_OK);jobdb_execution_t e={0};e.execution_id=9002;e.job_definition_id=1;e.state=JOBDB_EXEC_READY;e.eligible_at=(int64_t)time(NULL);assert(jobdb_execution_create(db,&e)==JOBDB_OK);jobdb_close(db);db=NULL;{int winners=0;for(unsigned p=1;p<=4;p++){sprintf(num,"%u",p);const char*args[]={argv[0],"claim-child",path,num,NULL};intptr_t h=_spawnv(_P_WAIT,argv[0],args);assert(h!=-1);int s=(int)h;assert(s==0||s==1);if(s==0)winners++;}assert(winners==1);}
...
#else
    if(strcmp(argv[1],"multiprocess")==0)return 0;
#endif
    return 2;
}
