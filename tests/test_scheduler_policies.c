...
#define TEST_LEASE_SECONDS 10
/* A claimed execution may need one lease expiry before restart recovery reclaims it. */
#define RESTART_RECOVERY_TIMEOUT_MS ((TEST_LEASE_SECONDS * 1000u) + 5000u)
...
static void wait_handled(jobdb_t *db, int count) { unsigned i; for (i = 0; i < RESTART_RECOVERY_TIMEOUT_MS / 10u && handled < count; ++i) pause_ms(10); if (handled < count) { jobdb_execution_t a={0},b={0}; (void)jobdb_execution_get(db,9001,&a); (void)jobdb_execution_get(db,9002,&b); fprintf(stderr,"queue timeout: handled=%d active=%d/r%llu queued=%d/r%llu\n",(int)handled,(int)a.state,(unsigned long long)a.revision,(int)b.state,(unsigned long long)b.revision); } assert(handled >= count); }
...
int main(void) {
    ...
    clean_db(); assert(jobdb_create("scheduler-policy-test", &db)==JOBDB_OK); assert(jobcore_create(db, 2, TEST_LEASE_SECONDS, &core)==JOBDB_OK); assert(jobcore_register_handler(core,42,handler,NULL)==JOBDB_OK);
    ...
    assert(jobdb_open("scheduler-policy-test",&db)==JOBDB_OK); assert(jobcore_create(db,1,TEST_LEASE_SECONDS,&core)==JOBDB_OK); assert(jobcore_register_handler(core,42,handler,NULL)==JOBDB_OK); assert(jobcore_start(core)==JOBDB_OK); pause_ms(300); assert(jobcore_schedule_get(core,1003,&s)==JOBDB_OK && s.occurrence_count==1 && s.next_fire_at==INT64_MAX); assert(jobcore_stop(core)==JOBDB_OK); jobcore_destroy(core); core=NULL;

    /* Persist one active and exactly one blocked occurrence, then prove restart releases it. */
    handled = 0;
    assert(jobcore_create(db,1,TEST_LEASE_SECONDS,&core)==JOBDB_OK); create_interval(core,1004,now+3600,2,0,JOBCORE_FIXED_RATE,JOBCORE_OVERLAP_QUEUE_ONE);
    ...
    jobcore_destroy(core); core=NULL; jobdb_close(db); db=NULL; assert(jobdb_open("scheduler-policy-test",&db)==JOBDB_OK); assert(jobcore_create(db,1,TEST_LEASE_SECONDS,&core)==JOBDB_OK); assert(jobcore_register_handler(core,42,handler,NULL)==JOBDB_OK); assert(jobcore_start(core)==JOBDB_OK); wait_handled(db,2);
