...
internal sealed class SqlStorageBridge : IDisposable
{
    ...
    internal SqlStorageBridge(SqlServerConnectionFactory connections, SqlServerOptions options)
    {
        ...
        Bind(ref v.RecordCreate,new RecordCreate(RecordCreateCallback));Bind(ref v.RecordGet,new RecordGet(RecordGetCallback));Bind(ref v.RecordUpdate,new RecordUpdate(RecordUpdateCallback));Bind(ref v.RecordDelete,new RecordDelete(RecordDeleteCallback));Bind(ref v.RecordFree,new RecordFree(RecordFreeCallback));Bind(ref v.ListIds,new ListIds(ListIdsCallback));
    ...
    private JobDbResult RecordUpdateCallback(IntPtr c,uint type,ulong id,ulong revision,IntPtr payload,uint size,IntPtr output)=>Run(()=>{using var db=Open();using var cmd=Command(db,null,$"UPDATE {_schema}.[Records] SET [Payload]=@p,[Revision]=[Revision]+1 WHERE [RecordType]=@t AND [RecordId]=@i AND [Revision]=@r");P(cmd,"@p",Bytes(payload,size));P(cmd,"@t",(int)type);P(cmd,"@i",L(id));P(cmd,"@r",L(revision));if(cmd.ExecuteNonQuery()!=1)return JobDbResult.Conflict;if(output!=IntPtr.Zero)Marshal.WriteInt64(output,unchecked((long)(revision+1)));return JobDbResult.Ok;});
    private JobDbResult RecordDeleteCallback(IntPtr c,uint type,ulong id,ulong revision)=>Run(()=>{using var db=Open();using var cmd=Command(db,null,$"DELETE FROM {_schema}.[Records] WHERE [RecordType]=@t AND [RecordId]=@i AND [Revision]=@r");P(cmd,"@t",(int)type);P(cmd,"@i",L(id));P(cmd,"@r",L(revision));return cmd.ExecuteNonQuery()==1?JobDbResult.Ok:JobDbResult.Conflict;});
    ...
    internal JobDbResult TestClaim(byte[] worker,long lease,out Execution execution){var w=new Worker{Bytes=worker};return ClaimCallback(IntPtr.Zero,ref w,0,lease,out execution);}internal JobDbResult TestStart(ulong id,byte[] worker,ulong fence){var w=new Worker{Bytes=worker};return StartCallback(IntPtr.Zero,id,ref w,fence,0,IntPtr.Zero);}internal JobDbResult TestFinalize(ulong id,byte[] worker,ulong fence){var w=new Worker{Bytes=worker};return FinalizeCallback(IntPtr.Zero,id,ref w,fence,7,0,0);}
    internal IReadOnlyList<BasaltExecutionInfo> ManagementListExecutions(int take,ulong after)=>ManagementListExecutions(new ExecutionQuery { Take=take, AfterExecutionId=after });
    internal IReadOnlyList<BasaltExecutionInfo> ManagementListExecutions(ExecutionQuery query)
    {
        if(query==null)throw new ArgumentNullException(nameof(query));if(query.Take<1||query.Take>1000)throw new ArgumentOutOfRangeException(nameof(query.Take));
        var where=new List<string>{"[ExecutionId]>@after"};using var db=Open();using var c=Command(db,null,string.Empty);P(c,"@take",query.Take);P(c,"@after",L(query.AfterExecutionId));
        if(query.States!=null&&query.States.Count!=0){var names=new List<string>();int index=0;foreach(var state in query.States){string name="@state"+index++;names.Add(name);P(c,name,(int)state);}where.Add("[State] IN ("+string.Join(",",names)+")");}
        if(query.JobDefinitionId.HasValue){where.Add("[JobDefinitionId]=@job");P(c,"@job",L(query.JobDefinitionId.Value));}if(query.ScheduleId.HasValue){where.Add("[ScheduleId]=@schedule");P(c,"@schedule",L(query.ScheduleId.Value));}if(query.WorkflowId.HasValue){where.Add("[WorkflowId]=@workflow");P(c,"@workflow",L(query.WorkflowId.Value));}if(query.CreatedFrom.HasValue){where.Add("[CreatedAt]>=@createdFrom");P(c,"@createdFrom",query.CreatedFrom.Value.ToUnixTimeSeconds());}if(query.CreatedTo.HasValue){where.Add("[CreatedAt]<=@createdTo");P(c,"@createdTo",query.CreatedTo.Value.ToUnixTimeSeconds());}
        c.CommandText=$"SELECT TOP (@take) * FROM {_schema}.[Executions] WHERE {string.Join(" AND ",where)} ORDER BY [ExecutionId]";var result=new List<BasaltExecutionInfo>();using var r=c.ExecuteReader();while(r.Read())result.Add(ToInfo(ReadExecution(r)));return result;
    }
    internal BasaltQueueStats ManagementGetQueueStats()
    {
        ulong[] counts=new ulong[12];using var db=Open();using var c=Command(db,null,$"SELECT [State],COUNT_BIG(*) FROM {_schema}.[Executions] GROUP BY [State]");using var r=c.ExecuteReader();while(r.Read()){int state=Convert.ToInt32(r[0]);if(state>=0&&state<counts.Length)counts[state]=Convert.ToUInt64(r[1]);}return new BasaltQueueStats(counts[0],counts[1],counts[2],counts[3],counts[4],counts[5],counts[6],counts[7],counts[8],counts[9],counts[10],counts[11]);
    }
    internal BasaltPerformanceStats ManagementGetPerformanceStats()
    {
        using var db=Open();using var c=Command(db,null,$"SELECT SUM(CASE WHEN [State]=7 THEN 1 ELSE 0 END),SUM(CASE WHEN [State] IN(8,9) THEN 1 ELSE 0 END),AVG(CASE WHEN [StartedAt]>0 AND [FinishedAt]>0 THEN CONVERT(float,[FinishedAt]-[StartedAt]) END),AVG(CASE WHEN [StartedAt]>0 THEN CONVERT(float,[StartedAt]-[CreatedAt]) END) FROM {_schema}.[Executions]");using var r=c.ExecuteReader();r.Read();ulong completed=r.IsDBNull(0)?0:Convert.ToUInt64(r[0]);ulong failed=r.IsDBNull(1)?0:Convert.ToUInt64(r[1]);TimeSpan? duration=r.IsDBNull(2)?null:TimeSpan.FromSeconds(Convert.ToDouble(r[2]));TimeSpan? wait=r.IsDBNull(3)?null:TimeSpan.FromSeconds(Convert.ToDouble(r[3]));var totals=ManagementGetStats();return new BasaltPerformanceStats(completed,failed,totals.RetriedTotal,duration,wait);
    }
    private static BasaltExecutionInfo ToInfo(Execution e)=>new(e.Id,e.JobType,e.ScheduleId,e.WorkflowId,e.State,e.CreatedAt,e.EligibleAt,e.StartedAt,e.FinishedAt,e.Priority,e.Attempt,e.MaxAttempts,e.Revision,e.LeaseExpiresAt,e.FencingToken,e.WorkerId);
    ...
    internal BasaltLedgerEntry ManagementGetLedger(ulong id){using var db=Open();using var c=Command(db,null,$"SELECT * FROM {_schema}.[Ledger] WHERE [ExecutionId]=@i");P(c,"@i",L(id));using var r=c.ExecuteReader();if(!r.Read())throw new BasaltException(JobDbResult.NotFound);long started=Convert.ToInt64(r[4]),finished=Convert.ToInt64(r[5]);return new BasaltLedgerEntry{ExecutionId=U(r[0]),JobDefinitionId=U(r[1]),ScheduleId=U(r[2]),WorkflowId=U(r[3]),StartedAt=started==0?(DateTimeOffset?)null:DateTimeOffset.FromUnixTimeSeconds(started),FinishedAt=finished==0?(DateTimeOffset?)null:DateTimeOffset.FromUnixTimeSeconds(finished),Duration=TimeSpan.FromSeconds(Convert.ToInt64(r[6])),Attempt=Convert.ToUInt32(r[7]),FinalState=(ExecutionState)Convert.ToUInt32(r[8]),ResultCode=Convert.ToInt32(r[9]),ErrorCode=Convert.ToInt32(r[10])};}
    ...
    internal void ManagementRequeue(ulong id,ulong revision){using var db=Open();using var tx=db.BeginTransaction();using var c=Command(db,tx,$"UPDATE {_schema}.[Executions] SET [State]=2,[EligibleAt]=DATEDIFF_BIG(SECOND,'19700101',SYSUTCDATETIME()),[Attempt]=0,[StartedAt]=0,[FinishedAt]=0,[WorkerId]=NULL,[LeaseExpiresAt]=0,[FencingToken]=0,[Revision]=[Revision]+1 WHERE [ExecutionId]=@i AND [Revision]=@r AND [State] IN(7,8,9,10,11)");P(c,"@i",L(id));P(c,"@r",L(revision));if(c.ExecuteNonQuery()!=1){tx.Rollback();throw new BasaltException(Exists(db,id)?JobDbResult.Conflict:JobDbResult.NotFound);}using var ledger=Command(db,tx,$"DELETE FROM {_schema}.[Ledger] WHERE [ExecutionId]=@i");P(ledger,"@i",L(id));ledger.ExecuteNonQuery();tx.Commit();}
