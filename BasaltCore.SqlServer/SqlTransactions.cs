...
internal static class SqlTransactions
{
    ...
    internal static JobDbResult Record(IntPtr handle,uint type,ulong id,byte[] payload){State s=Get(handle);return s.Owner.Safe(()=>{if(type==6){using var c=s.Owner.InternalCommand(s.Connection,s.Transaction,$"INSERT INTO {s.Owner.Schema}.[Receipts] VALUES(@i,@p)");SqlStorageBridge.Parameter(c,"@i",SqlStorageBridge.Long(id));SqlStorageBridge.Parameter(c,"@p",payload);c.ExecuteNonQuery();}else s.Owner.InternalInsertRecord(s.Connection,s.Transaction,type,id,payload);return JobDbResult.Ok;});}
    ...
    internal static JobDbResult Commit(IntPtr handle){State s=Get(handle);try{s.Transaction.Commit();s.Completed=true;return s.Owner.InternalCommitOutcomeLost()?JobDbResult.UnknownCommit:JobDbResult.Ok;}catch(Microsoft.Data.SqlClient.SqlException){return JobDbResult.UnknownCommit;}catch{return JobDbResult.Io;}}
